...
function addToCart(menu, price) {
    ...
    for(let i = 0; i < cart.children.length; i++){
        plus.addEventListener("click", (e) => {
            e.target.parentNode.parentNode.childNodes[1].
                childNodes[1].innerHTML = Number(e.target.parentNode.parentNode.childNodes[1].
                childNodes[1].textContent) + 1;;
        });
    }
    ...
    for(let i = 0; i < cart.children.length; i++){
        del[i].addEventListener("click", (e) => {
            e.target.parentNode.parentNode.remove();
            for(let k = 0; k < menuAndQuantity[0].length; k++){
                if(e.target.parentNode.parentNode.childNodes[0].
        firstChild.firstChild.textContent == menuAndQuantity[0][k]){
                    menuAndQuantity[1][k] = 0;
                }
            }
        });
    }
